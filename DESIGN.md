...
### Navigation
- Minimized index docked left over a solar-system menu: planets are click portals with hover ignition. Rows are skewed display type with amber slab sweeps and mono indices. Full game controls: arrows or WASD/HJKL move the index focus, Enter confirms, digits 1–6 quick-travel from anywhere in the app, brackets `[` `]` step to the previous/next section from inside any page, ESC returns to the index, and arrow keys hop between skill hexes and between award and credential tiles directionally, straight from the section body with no tabbing first. The side nav is always open on the menu screen. Sticky sysbar and status strip frame everything.
...
### Small screens
- Single column under 720px with the same amber voice throughout: the menu goes planet-only with the index column hidden and taps landing directly on planets, the constellation pans inside its map at full node size, awards and credentials both run as sideways touch rails, and the case lid wraps between items instead of breaking words. RETURN, dialog close, strip links, and the sound toggle all meet 44px touch targets.
...
- One beveled dark case holds everything behind a single lid (honor + credential counts): awards as OS-window photo cards (diamond pictogram tiles where no photo exists) and credentials as the unaltered original badge art on a horizontal rail that scrolls sideways when it overflows, numbered-disc sockets where art is unavailable. Dotted LED dividers label the two halves. Selecting a tile opens a focus-trapped dialog with the full-size photograph; Escape closes it and returns focus to the tile. Awards and credentials both navigate by arrow keys. Hover or keyboard focus floods the window with a circular LED-dot dither while the art lifts — the dither is the highlight, with no outline ring or outer halo. The fanned trio keeps its layout with no per-photo glow and no dither at all. Lid carries aria-expanded and the tray hides again on close. Instant under reduced motion. Images ship as WebP (lossless for badge art) so the case costs a fraction of the old PNG weight.
