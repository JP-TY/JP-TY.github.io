...
### Navigation
- Minimized index docked left over a solar-system menu: planets are click portals with hover ignition. Rows are skewed display type with amber slab sweeps and mono indices. Full game controls: arrows or WASD/HJKL move the index focus, Enter confirms, digits 1–6 quick-travel from anywhere in the app, brackets `[` `]` step to the previous/next section from inside any page, ESC returns to the index, and arrow keys also hop between skill hexes directionally. The side nav is always open on the menu screen. Sticky sysbar and status strip frame everything.
